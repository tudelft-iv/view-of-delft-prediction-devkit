...
from vod.prediction.input_representation.utils import (convert_to_pixel_coords,
                                                       get_crops,
                                                       get_rotation_matrix,
                                                       smooth_rotation,
                                                       smooth_rotations)
...
def draw_agent_boxes(
    ...
    for instance_token, annotations in agent_history.items():
        ...
        rotations = smooth_rotations(annotations)
        for i, annotation in enumerate(annotations):
            # if annotation["category_name"] in [
            #    "human.cyclist.adult",
            #    "human.rider.adult",
            # ]:
            #    # do not draw riders
            ...
            #  annotation["rotation"] = rotations[i]
            box = get_track_box(
...
def draw_first_last_agent_boxes(
    center_agent_annotation: Dict[str, Any],
    center_agent_pixels: Tuple[float, float],
    agent_history: History,
    base_image: np.ndarray,
    ...
    for instance_token, annotations in agent_history.items():
        ...
        for i, annotation in enumerate(annotations):
            ...
            translation = annotation["translation"][:2]
            row_pixel, column_pixel = convert_to_pixel_coords(
                translation, (agent_x, agent_y), center_agent_pixels, resolution
            )
            # translations.append([row_pixel, column_pixel])
            translations.append([column_pixel, row_pixel])

            if i not in [0, num_points - 1]:
                continue

            # new_annotation["rotation"] = rotations[i]
            box = get_track_box(
                new_annotation, (agent_x, agent_y), center_agent_pixels, resolution
            )
        ...
        for start_pt, end_pt in zip(translations[:-1], translations[1:]):
            cv2.line(
                base_image,
                tuple(start_pt),
                tuple(end_pt),
                color,
                thickness=int(0.5 / resolution),
            )
...
class AgentBoxesWithPast(AgentRepresentation):
    """
    Represents the past sequence of agent states as a three-channel
    image with faded 2d boxes.
    """
