...
Please refer to the [tutorial](https://www.vod.org/nuimages#tutorials) for an introduction to the most important database tables.

![](https://www.vod.org/public/images/nuimages-schema.svg)
...
The attributes in nuImages are a superset of those in VOD.
```
attribute {
...
Contrary to VOD, all camera images come distorted and unrectified.
```
calibrated_sensor {
   "token":                   <str> -- Unique record identifier.
...
The categories in nuImages are the same as in VOD (w/o lidarseg), plus `flat.driveable_surface`.
```
category {
   "token":                   <str> -- Unique record identifier.
...
Therefore the coordinates **should not be compared across logs** or rendered on the semantic maps of VOD.
```
ego_pose {
   "token":                   <str> -- Unique record identifier.
