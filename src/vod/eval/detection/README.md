# VOD detection task
![VOD Detection logo](https://www.vod.org/public/images/tasks.png)
...
## Introduction
Here we define the 3D object detection task on VOD.
...
## Participation
The VOD detection [evaluation server](https://eval.ai/web/challenges/challenge-page/356/overview) is open all year round for submission.
...
After each challenge, the results will be exported to the VOD [leaderboard](https://www.vod.org/object-detection) shown above.
...
## Challenges
To allow users to benchmark the performance of their method against the community, we host a single [leaderboard](https://www.vod.org/object-detection) all-year round.
...
The fourth VOD detection challenge will be held at [ICRA 2021](http://www.icra2021.org/) as part of [The AI Driving Olympics: Evaluating Progress in Robotics through Standardized and Reproducible Benchmarks](https://driving-olympics.ai/).
...
For details, please refer to the [detection leaderboard](https://www.vod.org/object-detection).
...
The third VOD detection challenge will be held at [NeurIPS 2020](https://nips.cc/Conferences/2020/).
...
The second VOD detection challenge will be held at [ICRA 2020](https://www.icra2020.org/).
...
The first VOD detection challenge was held at CVPR 2019.
...
For more information see the [leaderboard](https://www.vod.org/object-detection).
...
```
Note that the detection classes may differ from the general VOD classes, as detailed below.

## Classes, attributes, and detection ranges
The VOD dataset comes with annotations for 23 classes ([details](https://www.vod.org/data-annotation)).
...
Below we show the table of detection classes and their counterparts in the VOD dataset.
For more information on the classes and their frequencies, see [this page](https://www.vod.org/vod#data-annotation).

|   VOD detection class|   VOD general class                  |
|   ---                     |   ---                                     |
|   void / ignore           |   animal                                  |
...
Below we list which VOD classes can have which attributes.
Note that some annotations are missing attributes (0.4% of all sample_annotations).

For each VOD detection class, the number of annotations decreases with increasing range from the ego vehicle, 
...
|   VOD detection class    |   Attributes                                          | Detection range (meters)  |
|   ---                         |   ---                                                 |   ---                     |
|   barrier                     |   void                                                |   30                      |
...
## Evaluation metrics
Below we define the metrics for the VOD detection task.
...
### Configuration
The default evaluation metrics configurations can be found in `vod/eval/detection/configs/detection_cvpr_2019.json`. 

## Leaderboard
VOD will maintain a single leaderboard for the detection task.
...
We define three such filters here which correspond to the tracks in the VOD detection challenge.
