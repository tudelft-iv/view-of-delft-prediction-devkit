# VOD lidar panoptic segmentation and tracking task
![VOD lidar panoptic logo](https://www.vod.org/public/images/panoptic_challenge.jpg)
...
## Introduction
We define the lidar panoptic segmentation and panoptic tracking tasks on Panoptic VOD. This challenge is a
...
the [Panoptic VOD paper](https://arxiv.org/pdf/2109.03805.pdf) for more details.
...
## Participation
The Panoptic VOD challenge [evaluation server](https://eval.ai/web/challenges/challenge-page/1243/overview) is
...
each challenge, the results will be exported to the [Panoptic VOD leaderboard](https://www.vod.org/panoptic).
This is the only way to benchmark your method against the test dataset. We require that all participants send the
following information to VOD@motional.com after submitting their results on EvalAI:
- Team name
- Method name
- Authors
- Affiliations
...
[Panoptic VOD leaderboard](https://www.vod.org/panoptic) with filters for different task tracks all year
...
The first Panoptic VOD challenge will be held at [NeurIPS 2021](https://nips.cc/Conferences/2021/).
...
For more information see the [leaderboard](https://www.vod.org/panoptic). Note that the
...
* Any attempt to circumvent these rules will result in a permanent ban of the team or company from all VOD
challenges.

## Ground truth format
A ground truth label file named `{token}_panoptic.npz` is provided for each sample in the Panoptic VOD dataset.
...
label_file_path = /data/sets/vod/panoptic/v1.0-mini/{token}_panoptic.npz
...
Note that the lidar panoptic challenge classes differ from the general VOD classes, as detailed below.

## Classes
The Panoptic VOD dataset comes with annotations for 32 classes
([details](https://www.vod.org/data-annotation)).
Some of these only have a handful of samples. Just as for the VOD-lidarseg dataset, we merge similar classes and
remove rare classes. This results in 10 thing classes and 6 stuff classes for the lidar panoptic challenge.
Below we show the table of panoptic challenge classes and their counterparts in the Panoptic VOD dataset.
For more information on the classes and their frequencies, see
[this page](https://www.vod.org/vod#data-annotation).
...
[Panoptic VOD paper](https://arxiv.org/pdf/2109.03805.pdf) for more details of PAT metric.
...
## Leaderboard
VOD will maintain a single panoptic leaderboard with filters to split 4 specific tracks: Segmentation-lidar,
...
* Only lidar panoptic annotations from Panoptic VOD are allowed.
...
* All VOD, Panoptic VOD, VOD-lidarseg and nuImages annotations are allowed.
...
VOD, calibration parameters, ego poses, `location`, `timestamp`, `num_lidar_pts`, `num_radar_pts`, `translation`,
